    ...
    def execute(self, userdata): 
        ...
        if view:
            ...
            if rotate:
                ...
                current_state = vals2sid(ol = 1, eefl = 1, pred = color, listst = 2)
                return 'success'
            else:
                userdata.counter += 1
                return 'failed'
...
class Placeonconveyor(State):
    ...
    def execute(self, userdata): 
        ...
        rospy.sleep(0.05)
        if place:
            gr = gripper_to_pos(0, 60, 200, False)    # GRIPPER TO POSITION 0
            lift = pnp.liftgripper()
            rospy.sleep(0.01)
            if lift:
                userdata.counter = 0
...
class Detach_object(State):
    ...
    def execute(self, userdata): 
        ...
        if userdata.counter >= 50:
            ...
            if gr:
                userdata.counter = 0
                current_state = vals2sid(ol = 0, eefl = 2, pred = 2, listst = 2)
                return 'success'
            else:
                userdata.counter += 1
                return 'failed'
...
''' NOTE: The main function below is outadated now because many changes have been made in the
        states. You may have to update the code below to test the state machine '''
